...
----
In `openspec-workshop/inventory` muss dann noch der OpenAI Api Key bei `[all:vars]` hinzugefügt werden:
OPENAI_API_KEY=...

Danach kann das ansible-playbook ausgeführt werden.
[source,shell]
----
ansible-playbook -i openspec-workshop/inventory -f 10 --tags execute playbook.yaml
----

## Verbinden mit Webtop

Auf jeder Instanz läuft ein Web-GUI z.B: https://3.71.32.5/. Siehe Datei `openspec-workshop/readme.txt` für die URLs
